...
import { X, ArrowLeft, ChevronLeft, ChevronRight, RotateCcw, Loader2, Lock, AlertCircle, Bookmark, TrendingUp } from 'lucide-react'
import clsx from 'clsx'
import type { Reel } from '@/lib/types'
import type { CompetitorGapIdea, Language, LockedContent } from '@/lib/ideas-types'
import type { ScrapedTrend } from '@/app/api/scrape-trends/route'
  ...
  return (
    <div className="rounded-2xl border border-[#262626] bg-[#111]/50 overflow-hidden animate-pulse">
      ...
      <div className="p-5 space-y-3">
        <div className="flex gap-2"><div className="h-5 w-24 rounded-full bg-[#1c1c1c]" /><div className="h-5 w-16 rounded-full bg-[#1c1c1c]" /></div>
...
export default function TrendingTopicsPanel({ myReels, onClose, onLock }: Props) {
  ...
  // Step 1: fetch trends, then generate ideas
  useEffect(() => {
    async function init() {
      try {
        const tRes = await fetch('/api/scrape-trends')
        const tJson = await tRes.json()
        if (!tRes.ok) throw new Error(tJson.error ?? 'Failed to fetch trends')
        const fetchedTrends: ScrapedTrend[] = tJson.trends
        setTrends(fetchedTrends)
        setTrendsLoading(false)
        setIdeasLoading(true)

        const iRes = await fetch('/api/trending-ideas', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ myReels, trends: fetchedTrends }),
        })
        const iJson = await iRes.json()
        if (!iRes.ok) throw new Error(iJson.error ?? 'Failed to generate ideas')
        setIdeas(iJson.ideas)
        const count = iJson.ideas.length
  ...
  const refreshIdea = async (pos: number) => {
    ...
    try {
      const res = await fetch('/api/trending-ideas', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ myReels, trends, count: 1, excludeTitles: ideas.map(i => i.title) }),
      })
      const json = await res.json()
  ...
  const isLoading = trendsLoading || ideasLoading
  ...
  const headerTitle = view === 'ideas' ? 'Trending Topics' : view === 'hook' ? 'Craft Your Hook' : 'Generate Script'
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#262626] flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {view === 'ideas' && <p className="text-xs text-[#555] mt-0.5">Ideas built on today&apos;s trending topics in India</p>}
              {view === 'hook' && selectedIdea && (
                <p className="text-xs text-[#555] mt-0.5 flex items-center gap-1.5">
                  <TrendingUp className="w-3 h-3 text-white" />
                  <span className="text-white font-medium">{selectedIdea.competitorName}</span>
                  <span>·</span>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {view === 'ideas' && (
            <>
              ...
              {isLoading && (
                <p className="text-xs text-[#555] mb-4 flex items-center gap-2">
                  <Loader2 className="w-3.5 h-3.5 animate-spin text-white" />
                  {trendsLoading ? 'Fetching today\'s trends in India…' : 'Building ideas from trends…'}
                </p>
              )}
                    ...
                    <div key={`${pos}-${idea.id}`} className="rounded-2xl border border-[#262626] bg-[#111]/50 flex flex-col overflow-hidden hover:border-violet-500/40 hover:bg-[#111]/80 transition-all duration-200">
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        <div className="flex items-center gap-2 flex-wrap mb-3">
                          <span className="text-[10px] font-bold text-white bg-violet-400/10 border border-violet-400/20 px-2 py-0.5 rounded-full flex items-center gap-1">
                            <TrendingUp className="w-2.5 h-2.5" />{idea.competitorName}
                          </span>
                        ...
                        <div className="mb-2">
                          <p className="text-[10px] font-semibold text-[#555] uppercase tracking-wider mb-1">Most creators miss</p>
              ...
              {language === null ? (
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-white bg-violet-500/10 border border-violet-500/20 px-2.5 py-1 rounded-full uppercase tracking-wider">{LANG_LABELS[language]}</span>
                  ...
                  {scriptLoading && <div className="flex items-center justify-center gap-2 py-10 text-[#555]"><Loader2 className="w-4 h-4 animate-spin text-white" /><span className="text-xs">Writing your script…</span></div>}
