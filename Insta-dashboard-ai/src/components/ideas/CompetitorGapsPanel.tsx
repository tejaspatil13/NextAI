...
import {
  X, ArrowLeft, ChevronLeft, ChevronRight, RotateCcw,
  Loader2, Lock, AlertCircle, Bookmark,
} from 'lucide-react'
    ...
    <div className="rounded-2xl border border-[#262626] bg-[#111]/50 overflow-hidden animate-pulse">
      ...
      <div className="p-5 space-y-3">
        <div className="flex gap-2">
          <div className="h-5 w-20 rounded-full bg-[#1c1c1c]" />
...
export default function CompetitorGapsPanel({ myReels, competitorReels, onClose, onLock, initialIdea }: Props) {
  const [view, setView] = useState<View>(initialIdea ? 'hook' : 'ideas')

  const [ideasLoading, setIdeasLoading] = useState(!initialIdea)
  ...
  const [selectedIdea, setSelectedIdea] = useState<CompetitorGapIdea | null>(initialIdea ?? null)
  const [hookLoading, setHookLoading] = useState(false)
  const [hookError, setHookError] = useState<string | null>(null)
  // hookPool grows as user requests more; poolIdx tracks current position
  ...
  useEffect(() => {
    if (initialIdea) {
      selectIdea(initialIdea)
      return
    }
    async function fetchIdeas() {
      try {
        const res = await fetch('/api/competitor-ideas', {
          method: 'POST',
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(t)
  }, [hookAnimation, hookPool, poolIdx])
  ...
  const refreshIdea = async (pos: number) => {
    ...
    try {
      ...
      const res = await fetch('/api/competitor-ideas', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ myReels, competitorReels, count: 1, excludeTitles: seenTitles }),
      })
      const json = await res.json()
      ...
      if (newIdea) {
        setIdeas(prev => {
          const updated = [...prev]
          updated[targetIdx] = newIdea
          return updated
        })
      }
    } catch {
      // keep existing idea on error
    } finally {
  ...
  const creatorStyle = (() => {
    const withTranscript = myReels.find(r => r.transcript)
    if (withTranscript?.transcript) return withTranscript.transcript.slice(0, 150)
    return 'energetic creator voice'
  })()
  ...
  const goNextHook = () => {
    ...
    if (poolIdx < hookPool.length - 1) {
      setPoolIdx(p => p + 1)
    } else {
      fetchNextHooks(selectedIdea)
    }
  }

  const goPrevHook = () => {
    if (poolIdx > 0) setPoolIdx(p => p - 1)
  }
  ...
  const headerTitle =
    view === 'ideas' ? 'Competitor Gaps' :
    view === 'hook' ? 'Craft Your Hook' :
    'Generate Script'
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#262626] flex-shrink-0">
          <div className="flex items-center gap-3">
            {view !== 'ideas' && (
              <button
                onClick={goBack}
                className="w-8 h-8 rounded-lg bg-[#1c1c1c] hover:bg-[#222] flex items-center justify-center transition-colors group"
              >
            ...
            <div>
              ...
              {view === 'ideas' && (
                <p className="text-xs text-[#555] mt-0.5">AI-generated gaps your competitors consistently miss</p>
              )}
              {view === 'hook' && selectedIdea && (
                <p className="text-xs text-[#555] mt-0.5 flex items-center gap-1.5">
                  <span className="text-rose-400 font-medium">{selectedIdea.competitorName}</span>
                  <span>·</span>
                  <span className="line-clamp-1">{selectedIdea.title}</span>
                </p>
              )}
              {view === 'script' && (
                <p className="text-xs text-[#555] mt-0.5">Hook locked · choose language · generate</p>
              )}
            </div>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg bg-[#1c1c1c] hover:bg-[#222] flex items-center justify-center transition-colors ml-3 flex-shrink-0"
          >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {ideasLoading
                  ...
                  : visibleIdeas.map((idea, pos) => idea && (
                    <div
                      key={`${pos}-${idea.id}`}
                      className="rounded-2xl border border-[#262626] bg-[#111]/50 flex flex-col overflow-hidden hover:border-violet-500/40 hover:bg-[#111]/80 transition-all duration-200"
                    >
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        <div className="flex items-center gap-2 flex-wrap mb-3">
                          ...
                          <span className={clsx(
                            ...
                            {idea.estimatedImpact === 'high' ? 'HIGH IMPACT' : 'MEDIUM IMPACT'}
                          </span>
                        </div>
                        ...
                        <div className="mb-2">
                          <p className="text-[10px] font-semibold text-[#555] uppercase tracking-wider mb-1">Their gap</p>
                        ...
                        <p className="text-[10px] text-[#555] italic leading-relaxed mb-4 flex-1">
                        ...
                        <div className="space-y-2 mt-auto">
                          <button
                            ...
                            className="w-full text-xs font-bold ig-gradient-bg hover:opacity-90 text-white py-2.5 rounded-xl transition-colors"
                          >
                            Work on This →
                          </button>
                          <button
                            onClick={() => refreshIdea(pos)}
            ...
            <div className="max-w-lg mx-auto flex flex-col gap-4">
              {hookLoading && (
                <div className="flex flex-col gap-3 animate-pulse">
                  <div className="h-3 w-48 rounded bg-[#1c1c1c]" />
                  <div className="h-24 w-full rounded-xl bg-[#1c1c1c]" />
                  <div className="h-10 w-full rounded-xl bg-[#1c1c1c]" />
                  <div className="h-10 w-full rounded-xl bg-[#1c1c1c]" />
                </div>
              )}
              ...
              {!hookLoading && hookPool.length > 0 && (
                <>
                  <div className={clsx(
                    ...
                    'bg-[#1c1c1c]/60',
                    isAnimating
                      ? 'border border-green-500/60 shadow-green-500/10 shadow-xl'
                      : 'border border-[#262626]/50'
                  )}>
                    {isAnimating && (
                      <div className={clsx(
                        'absolute top-3 right-3 transition-all duration-300',
                        hookAnimation === 'locked' ? 'scale-125' : 'scale-100'
                      )}>
                        <div className={clsx(
                          'w-7 h-7 rounded-full flex items-center justify-center',
                          hookAnimation === 'locked'
                    ...
                    <p className="text-sm text-white italic leading-relaxed pr-8">
                      &ldquo;{hookPool[poolIdx]}&rdquo;
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={goPrevHook}
                      disabled={isAnimating || poolIdx === 0}
                      className="w-9 h-9 rounded-lg bg-[#1c1c1c] hover:bg-[#222] flex items-center justify-center transition-colors disabled:opacity-30 flex-shrink-0"
                    >
                      <ChevronLeft className="w-4 h-4 text-[#a8a8a8]" />
                    </button>
                    <button
                      onClick={goNextHook}
                      disabled={isAnimating || hookFetching}
                      className="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold py-2 rounded-lg border text-white bg-[#833AB4]/10 hover:bg-[#833AB4]/20 border-[#833AB4]/25 transition-colors disabled:opacity-50"
                    >
                      {hookFetching
                        ? <><Loader2 className="w-3 h-3 animate-spin" /> Generating…</>
                    ...
                    <span className="text-[10px] text-[#444] tabular-nums flex-shrink-0">
                      {poolIdx + 1}/{hookPool.length}
                    </span>
                  </div>

                  <button
                    onClick={() => setHookAnimation('locking')}
                    disabled={isAnimating || hookFetching}
                    className={clsx(
                      'w-full py-3 rounded-xl font-bold text-sm transition-all duration-300',
                      hookAnimation === 'locked'
                        ? 'bg-green-600 text-white'
                        : 'ig-gradient-bg hover:opacity-90 text-white disabled:opacity-60'
                    )}
                  >
            ...
            <div className="max-w-lg mx-auto flex flex-col gap-4">
              <div className="rounded-xl border border-green-500/40 bg-green-500/5 p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#e0e0e0] italic leading-relaxed">&ldquo;{selectedHook}&rdquo;</p>
                </div>
              </div>
              ...
              {language === null ? (
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {LANGS.map(l => (
                      <button
                        ...
                        className="flex flex-col items-center gap-2 py-4 rounded-2xl ig-hover-btn bg-[#111] border border-[#262626] hover:border-[#833AB4]/50 transition-all duration-200 group"
                      >
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => { setLanguage(null); setScript('') }}
                  ...
                  {!script && !scriptLoading && !scriptError && (
                    <button
                      onClick={() => generateScript(language)}
                      className="w-full py-3 rounded-xl ig-gradient-bg hover:opacity-90 text-white font-bold text-sm transition-all shadow-lg shadow-[#833AB4]/20"
                    >
                      Generate Script
                    </button>
                  )}
                  ...
                  {scriptLoading && (
                    <div className="flex items-center justify-center gap-2 py-10 text-[#555]">
                  ...
                  {script && !scriptLoading && (
                    <>
                      ...
                      <button
                        onClick={() => generateScript(language)}
                        className="w-full flex items-center justify-center gap-1.5 text-xs font-semibold py-2.5 rounded-xl border text-[#a8a8a8] bg-[#1c1c1c] hover:bg-[#262626] hover:text-white border-[#262626] transition-colors"
                      >
                      ...
                      {onLock && (
                        <button
                          onClick={() => {
                            if (!selectedIdea) return
                            setLocked(true)
                            onLock({ idea: selectedIdea, hook: selectedHook, script, language, lockedAt: new Date().toISOString() })
                          }}
                          disabled={locked}
                          className={clsx(
                            ...
                            locked
                              ...
                              : 'ig-gradient-bg hover:opacity-90 text-white'
                          )}
                        >
                          <Bookmark className="w-4 h-4" />
                          {locked ? 'Saved to Taskbar ✓' : 'Lock & Save to Taskbar'}
                        </button>
                      )}
                    </>
                  )}
